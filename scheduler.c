...
static bool tiebreaker(Process *a, Process *b, Algorithm algorithm);
...
static bool tiebreaker(Process *a, Process *b, Algorithm algorithm) {
    ...
    // Tie-breaker 1: higher priority wins
    if (a->priority != b->priority)
        return a->priority > b->priority;

    // Tie-breaker 2: lower pid wins
    return a->pid < b->pid;
}
...
void handle_arrivals(Process *processes, int process_count, int current_time, Algorithm algorithm,
                   int *arrived_indices, int *arrival_count) {
    // Find and record processes that have arrived at the current time
    ...
    *arrival_count = 0; // Initialize arrival count
    ...
    for (int i = 0; i < process_count; i++) {
        ...
        // Record arrivals exactly at this time (and not already completed)
        if (p->arrival_time == current_time && p->state != COMPLETED) {
...
void handle_rr_quantum_expiry(Process *processes, CPU *cpus, int cpu_count, int time_quantum,
                           ReadyQueue *ready_queue, int current_time) {
    // Move Round Robin processes back to the queue when their quantum expires
    ...
    for (int c = 0; c < cpu_count; c++) {
        ...
        if (p->quantum_used >= time_quantum) {
            // pre empt and put back in circular queue
            p->state = READY;
            p->quantum_used = 0;
 ...
 */
 void handle_srtf_preemption(Process *processes, int process_count, CPU *cpus, int cpu_count, int current_time) {
    // Build a list of runnable processes: arrived, not completed, remaining > 0
    ...
    bool chosen[MAX_PROCESSES];
    for (int i = 0; i < MAX_PROCESSES; i++) chosen[i] = false;

    int desired[MAX_PROCESSES];
    int desired_count = 0;

    int limit = cpu_count;
    if (limit > run_cnt) limit = run_cnt;
    ...
    for (int k = 0; k < limit; k++) {
        ...
        for (int r = 0; r < run_cnt; r++) {
            ...
            Process *cand = &processes[pi];
            if (tiebreaker(cand, best, SRTF)) {
                best = cand;
                best_idx = pi;
            }
        }
        ...
        if (best_idx != -1) {
            ...
            desired[desired_count++] = best_idx;
        }
    }

    // Preempt any CPU whose current process isn't in desired set
    for (int c = 0; c < cpu_count; c++) {
        Process *cur = cpus[c].current_process;
        if (cur == NULL) continue;

        bool keep = false;
        for (int d = 0; d < desired_count; d++) {
            if (&processes[desired[d]] == cur) {
                keep = true;
                break;
            }
        }
...
void assign_processes_to_idle_cpus(Process *processes, int process_count, CPU *cpus, int cpu_count,
                                Algorithm algorithm, ReadyQueue *ready_queue, int current_time) {
    // Select and assign processes to idle CPUs according to the chosen algorithm
    ...
    if (algorithm == RR) {
        for (int c = 0; c < cpu_count; c++) {
            if (cpus[c].current_process != NULL) continue; // alr running

            int idx = dequeue(ready_queue);
            while (idx != -1) {
                Process *p = &processes[idx];

                if (p->state != COMPLETED && p->remaining_time > 0 && p->state != RUNNING) {
                    cpus[c].current_process = p;
                    p->state = RUNNING;

                    if (p->start_time == -1) p->start_time = current_time;
                    if (p->response_time == -1) p->response_time = current_time - p->arrival_time;

                    p->quantum_used = 0; // new time slice begins
                    break;
                }
                idx = dequeue(ready_queue);
            }
        }
        return;
    }

    if (algorithm != FCFS && algorithm != SJF && algorithm != SRTF) {
        return;
    }
    ...
    for (int c = 0; c < cpu_count; c++) {
        ...
        for (int i = 0; i < process_count; i++) {
            ...
            if (p->arrival_time <= current_time &&
                ...
                if (tiebreaker(p, winner, algorithm)) {
                    winner = p;
                }
            }
        }
 ...
 */
void update_waiting_times(Process *processes, int process_count, int current_time) {
    // Increment waiting_time for processes that have arrived but are not running
    (void)current_time;

    for (int i = 0; i < process_count; i++) {
        Process *p = &processes[i];

        // wait increments if process has arrived, but is not running or completed
        if ((p->arrival_time) <= current_time && p->state != RUNNING && p->state != COMPLETED && p->remaining_time > 0) {
            p->waiting_time++;
        }
    }
}
...
void execute_processes(Process *processes, int process_count, CPU *cpus, int cpu_count,
    ...
    for (int c = 0; c < cpu_count; c++) {
        ...
        // If finished, mark completion and free CPU
        if (p->remaining_time == 0) {
